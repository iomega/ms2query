...
If your files contain many MS2 spectra per feature it is advised to first reduce the number of MS2 spectra by clustering or feature selection.
We therefore advice to first use MZMine for preprocessing and use the MGF file of the FBMN output of MZMine, see https://ccms-ucsd.github.io/GNPSDocumentation/featurebasedmolecularnetworking-with-mzmine2/.

#### Running MS2Query
After downloading a default library MS2Query can be run on your MS2 spectra. 
Specify the location where your spectra are stored. 
...
To run MS2Query on your own created library run. Check out the instructions under Run MS2Query. Both command line and the code version should work.
For the SQLite file, S2V embeddings file and the ms2ds embeddings file. The files just generated by you should be used.
